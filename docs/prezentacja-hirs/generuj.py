"""Prezentacja sprzedażowa HiRS — Hospital Information Retrieval System.

DZIESIĘĆ slajdów, treść odpowiada wydaniu dla ZCO (`../prezentacja`), ale bez nazwy
klienta: adresatem jest szpital w ogóle. Wobec tamtego wydania:
  * nazwa systemu HiRS zamiast nazwy jednego wdrożenia, przykłady szpitalne
    (akredytacja, procedury kliniczne, zdarzenia niepożądane), ale nieprzypisane
    do żadnej placówki,
  * zamiast zrzutów ekranu — MAKIETY rysowane stylami, z danymi wymyślonymi.
    Nie pokazujemy ani dokumentów klienta, ani zawartości instancji demonstracyjnej,
  * slajdów jest dziesięć, nie jedenaście: warunki handlowe i kontakt stoją razem
    na ostatnim, bo cennik to trzy wiersze i nie potrzebuje własnej strony.

Kolory i ich rola (paleta aplikacji, sprawdzona walidatorem, tryb jasny):
  #1d2a4d  granat — tło paneli, nagłówki, kolor pisma na jasnym,
  #1fc8ba  turkus marki — WYŁĄCZNIE na granacie; na białym ma 2,04:1, poniżej progu 3:1,
  #0f9b8e  turkus przyciemniony — znaczniki na białym (3,4:1, ten sam odcień),
  #2563eb  niebieski — druga seria; para z #0f9b8e daje ΔE 21,3 przy zaburzeniach
           widzenia barw.
...
same wymiary przy 96 dpi — jeden slajd na jedną stronę PDF.
    ...
    python makiety_png.py    # makiety do PPTX (wymaga Edge)
...
NAZWA = "HiRS"
PODTYTUL_NAZWY = "Hospital Information Retrieval System"
DATA = "24 sierpnia 2026"
WYKONAWCA = "Polmedi Group sp. z o.o., Poznań"

# `NIEBIESKI` niesie tu rolę koloru wiodącego (nagłówki, ciemne panele) — w palecie
# aplikacji pełni ją GRANAT. Nazwa zmiennej zostaje wspólna z pozostałymi generatorami,
# żeby arkusz stylów i generator PPTX działały bez przeróbek.
NIEBIESKI = "#1d2a4d"
TURKUS = "#1fc8ba"
...
KROKI_SCIEZKI = [
    ("1", "Pytanie", "zwykłym językiem, np. „jak zgłosić zdarzenie niepożądane?”"),
...
def graf_kontakt_pasek():
    """Kontakt w jednym wierszu — wersja na slajd, który dzieli miejsce z cennikiem.

    Duże wizytówki miały sens, gdy kontakt zajmował cały slajd (tak jest
    w wydaniu jedenastoslajdowym, `../prezentacja-uniwersalna`).
    Po połączeniu warunków i kontaktu w jeden nie mieszczą się: przy pierwszym
    złożeniu nachodziły na pasek puenty, ucinając numery telefonów. Tu ta sama
    treść zajmuje jeden wiersz na osobę.
    """
    osoby = "".join(
        f"<div class='kp-osoba'><b>{imie}</b><span>{rola}</span>"
        f"<em>tel. {telefon}</em></div>" for imie, rola, telefon in OSOBY_KONTAKT)
    return f"<div class='kontakt-pasek'>{osoby}</div>"


# ---------------------------------------------------------------- makiety ekranów

def makieta_pliki():
    """Makieta eksploratora — rysowana, nie zrzut.

    Foldery i liczby są WYMYŚLONE. Nie pokazujemy ani dokumentów klienta, ani
    zawartości instancji demonstracyjnej: prezentacja jedzie do osób z zewnątrz,
    a nazwa folderu potrafi zdradzić więcej, niż się wydaje przy jej wpisywaniu.
    """
    foldery = [("Procedury kliniczne", 42), ("Akredytacja", 31), ("Kadry i płace", 27),
               ("Zarządzenia dyrektora", 24), ("BHP i RODO", 15), ("Szkolenia", 11)]
    kafle = "".join(
        f"<div class='m-folder'><div class='m-ikona'></div><b>{f}</b>"
        f"<span>dokumentów: {ile}</span></div>" for f, ile in foldery)
    return f"""
<div class='makieta'>
  <div class='m-belka'><span class='m-logo'>{NAZWA}</span></div>
  <div class='m-tresc'>
    <div class='m-menu'><span class='m-akt'>Pliki</span><span>Chat z AI</span><span>Wyszukiwanie</span></div>
...
<div class='makieta'>
  <div class='m-belka'><span class='m-logo'>{NAZWA}</span></div>
  <div class='m-tresc'>
    <div class='m-menu'><span>Pliki</span><span class='m-akt'>Chat z AI</span><span>Wyszukiwanie</span></div>
    <div class='m-panel'>
      <div class='m-pytanie'>jak zgłosić zdarzenie niepożądane?</div>
      <div class='m-odpowiedz'>Zgłoszenie składa się w ciągu 24 godzin od stwierdzenia
        zdarzenia, na formularzu karty zdarzenia niepożądanego <span class='m-zrodlo'>1</span>.
        Kartę przekazuje się pełnomocnikowi do spraw jakości, który rejestruje ją
        i wszczyna analizę przyczyn <span class='m-zrodlo'>2</span>.
        <div class='m-zrodla'>Dokumenty użyte w odpowiedzi:
          <div><span class='m-zrodlo'>1</span> Procedura zgłaszania zdarzeń niepożądanych.pdf (str. 2)</div>
          <div><span class='m-zrodlo'>2</span> Karta zdarzenia niepożądanego.docx (str. 1)</div>
        </div>
      </div>
      <div class='m-pytanie'>a kto prowadzi analizę przyczyn?</div>
    </div>
  </div>
</div>"""
...
SLAJDY = [
    {
        ...
        "notatka": "HiRS to Hospital Information Retrieval System. Trzy rzeczy do zapamiętania: "
                   "odpowiedzi wskazują dokument źródłowy, dane nie opuszczają budynku, "
                   "obsługa to wpisanie pytania.",
    },
    {
        ...
        "tresc": punkty(
            ...
            "Chmura odpada: dokumentacja szpitala nie może trafiać do zewnętrznego dostawcy",
        ),
    ...
    },
    {
        ...
        "tresc": punkty(
            ...
            "<b>Chat z AI</b> — pytanie zwykłym językiem, odpowiedź z odnośnikami do stron",
    ...
    },
    {
        ...
        "tresc": punkty(
            ...
            "Trzy ekrany: <b>Dashboard</b>, <b>Pliki</b>, <b>Chat z AI</b>. Administracja tylko "
            "dla administratora",
    ...
    },
    {
        ...
        "notatka": "Ten sam system sprawdzi się wszędzie tam, gdzie jest duży zasób dokumentów "
                   "wewnętrznych.",
    },
    {
        ...
        "tresc": graf_harmonogram() + punkty(
            ...
            "dokumentów",
            "<b>Kierunek rozwoju</b> (poza obecną wersją): powiązania między dokumentami i wersja "
            "obowiązująca, podgląd PDF na cytowanej stronie, logowanie kontem domenowym, "
    ...
    # Warunki i kontakt na JEDNYM slajdzie — cennik to trzy wiersze, a rozbicie na dwie
    # strony kazałoby wracać do cennika przy pytaniu o cenę na końcu rozmowy.
    {
        "tytul": "Warunki, następny krok i kontakt",
        "puenta": "Następny krok: dwutygodniowe uruchomienie próbne na Waszych dokumentach "
                  "— u Was, bez zobowiązania.",
        "tresc": graf_cennik() + graf_kontakt_pasek(),
        "notatka": "Nie proszę o decyzję zakupową, tylko o test na własnych dokumentach. "
                   "Jeśli po dwóch tygodniach kadry powiedzą, że to nie pomaga — rozstajemy się "
                   "bez kosztów. Do Piotra dzwonicie w sprawie wyceny i umowy, do mnie — "
                   "gdy pytanie jest techniczne.",
    },
]
...
/* kontakt w jednym wierszu — slajd „Warunki, następny krok i kontakt" */
.kontakt-pasek {{ display:flex; gap:24px; margin-top:18px; }}
.kp-osoba {{ flex:1; display:flex; align-items:baseline; gap:12px; background:var(--tlo);
            border:1px solid var(--linia); border-radius:10px; padding:12px 18px; }}
.kp-osoba b {{ font-size:20px; color:var(--niebieski); white-space:nowrap; }}
.kp-osoba span {{ font-size:14px; color:var(--szary); flex:1; }}
.kp-osoba em {{ font-size:19px; font-weight:600; font-style:normal; color:var(--turkus-ciemny);
               white-space:nowrap; }}
...
def main():
    html_path = os.path.join(KATALOG, "HiRS-prezentacja.html")
    pdf_path = os.path.join(KATALOG, "HiRS-prezentacja.pdf")
    with open(html_path, "w", encoding="utf-8") as f:
        f.write(render())
    ok = do_pdf(html_path, pdf_path)
    print(f"HiRS-prezentacja: {len(SLAJDY)} slajdów, {os.path.getsize(html_path)//1024} KB HTML, "
