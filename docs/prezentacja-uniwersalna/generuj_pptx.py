"""EDMund — ta sama prezentacja w formacie PowerPoint, do samodzielnego poprawiania.
...
NIEBIESKI = RGBColor(0x24, 0x48, 0xC8)
TURKUS = RGBColor(0x09, 0xAF, 0xAF)
TURKUS_CIEMNY = RGBColor(0x0A, 0x9A, 0x9A)
TEKST = RGBColor(0x46, 0x50, 0x50)
SZARY = RGBColor(0x7B, 0x85, 0x85)
LINIA = RGBColor(0xE2, 0xE8, 0xF0)
TLO = RGBColor(0xF7, 0xFA, 0xFB)
...
def graf_serwerownia(s, y):
    ...
    for i, (nazwa, opis) in enumerate(WARSTWY_SERWEROWNI):
        ...
        k = prostokat(s, x, y + 52, szer, 118, RGBColor(0x3A, 0x5C, 0xD2))
        k.line.fill.background()
...
def graf_kontakt(s, y):
    tekst(pole(s, MARGINES, y, 600, 26), "Czekają na Was:", 17, TEKST)
    y += 38
    szer = (1152 - 28) / 2
    for i, (imie, rola, telefon) in enumerate(OSOBY_KONTAKT):
        ...
        kolo.fill.solid()
        kolo.fill.fore_color.rgb = NIEBIESKI
        kolo.line.fill.background()
        kolo.shadow.inherit = False
        czlony = imie.split()
        tfi = kolo.text_frame
        ...
        tekst(tfi, (czlony[0][0] + czlony[-1][0]).upper(), 20, BIALY, bold=True,
              interlinia=1.0).alignment = PP_ALIGN.CENTER
        tf = pole(s, x + 126, y + 28, szer - 150, 90)
        tekst(tf, imie, 19, NIEBIESKI, bold=True, interlinia=1.15)
    ...
    tekst(pole(s, MARGINES, y + 16, 800, 26),
          "Polmedi Group sp. z o.o. · Poznań · polmedi.com", 13.5, NIEBIESKI, bold=True)
    return y + 50
...
GRAFIKI = {
    ...
    "Warunki i następny krok": graf_cennik,
    "Zapraszamy do kontaktu": graf_kontakt,
}
...
def main():
    ...
    cel = os.path.join(KATALOG, "EDMund-prezentacja.pptx")
    prs.save(cel)
    print(f"EDMund-prezentacja.pptx: {len(SLAJDY)} slajdów, {os.path.getsize(cel)//1024} KB")
